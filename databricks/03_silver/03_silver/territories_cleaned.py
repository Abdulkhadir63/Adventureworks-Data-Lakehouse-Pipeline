...
def transform_territories_silver_tier(df_raw):
    ...
    specifically optimized for the Territories dimension table.
    """
...
logger.info("Enforcing structural types prior to execution grain deduplication")
df_validated_batch = df_bronze_batch.filter(F.col("SalesTerritoryKey").isNotNull())
df_transformed = transform_territories_silver_tier(df_validated_batch)


# 5. DETERMINISTIC DEDUPLICATION & PROJECTIONS
logger.info("Executing windowed deduplication based on stable primary key grain")
...
# Columns perfectly matched with transformed properties
target_columns = [
    "SalesTerritoryKey",
    "Region",
    "Country",
...
if spark.catalog.tableExists(target_table):
    ...
    silver_delta_table.alias("target") \
        .merge(
            ...
            condition = "target.SalesTerritoryKey = updates.SalesTerritoryKey"
        ) \
