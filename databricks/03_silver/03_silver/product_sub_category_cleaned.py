...
# 2. MODULAR TRANSFORMATION FUNCTION (Fixed for Subcategory schema)
def transform_product_subcategory_silver_tier(df_raw):
    """
    Applies explicit type casting, standards mapping, and string trimming 
    specifically optimized for the Product Subcategory data pipeline.
    """
    logger.info("Applying product subcategory table transformation rules")
    return df_raw \
...
logger.info("Running product-subcategory schema transformations")
# FIX: Calling the correct local function matching this notebook's scope
df_transformed = transform_product_subcategory_silver_tier(df_deduplicated)
...
if spark.catalog.tableExists(target_table):
    ...
    silver_delta_table.alias("target") \
        .merge(
            ...
            condition = "target.ProductSubcategoryKey = updates.ProductSubcategoryKey"
        ) \
