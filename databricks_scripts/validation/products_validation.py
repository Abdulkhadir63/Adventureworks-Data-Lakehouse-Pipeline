...
null_p_key = metrics["null_p_key"] or 0
null_sub_key = metrics["null_sub_key"] or 0
null_sku = metrics["null_sku"] or 0
null_name = metrics["null_name"] or 0

negative_financials = metrics["negative_financials"] or 0
negative_margin_rows = metrics["negative_margin_rows"] or 0

# Total critical structural failures (excluding soft WARNING rules)
total_critical_failures = null_p_key + null_sub_key + null_sku + null_name + negative_financials + duplicate_rows
...
validation_rows = [
    ...
    (data_source, "Required Column: ProductKey", "PASS" if null_p_key == 0 else "FAIL", int(null_p_key), current_max_silver_ts),
    (data_source, "Required Column: ProductSubcategoryKey", "PASS" if null_sub_key == 0 else "FAIL", int(null_sub_key), current_max_silver_ts),
    (data_source, "Required Column: ProductSKU", "PASS" if null_sku == 0 else "FAIL", int(null_sku), current_max_silver_ts),
    (data_source, "Required Column: ProductName", "PASS" if null_name == 0 else "FAIL", int(null_name), current_max_silver_ts),
    (data_source, "Financial Logic: Non-Negative Values", "PASS" if negative_financials == 0 else "FAIL", int(negative_financials), current_max_silver_ts),
    (data_source, "Business Rule: Price Greater Than Cost", "PASS" if negative_margin_rows == 0 else "WARNING", int(negative_margin_rows), current_max_silver_ts)
]
...
if pipeline_passed == "FAIL":
    raise ValueError(
        ...
        f"Critical failures detected: {total_critical_failures} (Null Product Keys: {null_p_key}, Null Subcategory Keys: {null_sub_key}, Invalid SKUs: {null_sku}, Invalid Names: {null_name}, Negative Financials: {negative_financials}, Duplicates: {duplicate_rows}), Total Rows: {total_rows}. "
