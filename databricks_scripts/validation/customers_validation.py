...
primary_grain_window = Window.partitionBy("CustomerKey")
...
metrics = df_flagged.select(
    ...
    F.sum("duplicate_flag").alias("duplicate_pks"),
...
duplicate_pks = metrics["duplicate_pks"] or 0
null_pks = metrics["null_pks"] or 0
malformed_emails = metrics["malformed_emails"] or 0

total_critical_failures = null_pks + duplicate_pks + malformed_emails
...
validation_rows = [
    (data_source, "Row Count Check", "PASS" if total_rows > 0 else "FAIL", int(total_rows if total_rows == 0 else 0), current_max_silver_ts),
    (data_source, "Unique Primary Key Grain", "PASS" if duplicate_pks == 0 else "FAIL", int(duplicate_pks), current_max_silver_ts),
    (data_source, "Valid Primary Keys", "PASS" if null_pks == 0 else "FAIL", int(null_pks), current_max_silver_ts),
    (data_source, "Email Format Check", "PASS" if malformed_emails == 0 else "FAIL", int(malformed_emails), current_max_silver_ts)
]
...
if pipeline_passed == "FAIL":
    raise ValueError(
        ...
        f"Critical failures detected: {total_critical_failures} (Null PKs: {null_pks}, Duplicates: {duplicate_pks}, Malformed Emails: {malformed_emails}), Total Rows: {total_rows}. "
