...
invalid_names = metrics["invalid_names"] or 0

total_critical_failures = null_pks + duplicate_rows + invalid_names
...
validation_rows = [
    ...
    (data_source, "Mandatory Core Columns", "PASS" if null_pks == 0 else "FAIL", int(null_pks), current_max_silver_ts),
    (data_source, "Required String Columns", "PASS" if invalid_names == 0 else "FAIL", int(invalid_names), current_max_silver_ts)
]
...
if pipeline_passed == "FAIL":
    raise ValueError(
        ...
        f"Critical failures detected: {total_critical_failures} (Null PKs: {null_pks}, Duplicates: {duplicate_rows}, Invalid Names: {invalid_names}), Total Rows: {total_rows}. "
