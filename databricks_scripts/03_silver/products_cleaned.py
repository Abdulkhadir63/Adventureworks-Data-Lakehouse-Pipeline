...
def transform_products_silver_tier(df_raw):
    """
    Applies explicit type casting, standards mapping, string cleansing, 
    and appends silver metadata for Products records.
    """
    return df_raw \
        .withColumn("ProductKey", F.col("ProductKey").cast(IntegerType())) \
        .withColumn("ProductSubcategoryKey", F.col("ProductSubcategoryKey").cast(IntegerType())) \
        .withColumn("ProductSKU", F.trim(F.coalesce(F.col("ProductSKU"), F.lit("Unknown")))) \
        .withColumn("ProductName", F.trim(F.upper(F.coalesce(F.col("ProductName"), F.lit("Unknown"))))) \
        .withColumn("ModelName", F.trim(F.upper(F.coalesce(F.col("ModelName"), F.lit("Unknown"))))) \
        .withColumn("ProductDescription", F.trim(F.upper(F.coalesce(F.col("ProductDescription"), F.lit("Unknown"))))) \
        .withColumn("ProductColor", F.trim(F.upper(F.coalesce(F.col("ProductColor"), F.lit("NA"))))) \
        .withColumn("ProductSize", F.trim(F.upper(F.coalesce(F.col("ProductSize"), F.lit("NA"))))) \
        .withColumn("ProductStyle", F.trim(F.upper(F.coalesce(F.col("ProductStyle"), F.lit("NA"))))) \
...
if not spark.catalog.tableExists(target_table):
    ...
    empty_transformed_df = transform_products_silver_tier(empty_schema_df)
...
# -------------------------------------------------------------------------
def process_products_micro_batch(df_micro_batch, batch_id):
    ...
    df_validated_batch = df_micro_batch.filter(
        F.col("ProductKey").isNotNull()
    )

    # WINDOW DEDUPLICATION ON PRIMARY KEY (ProductKey)
    window_spec = Window.partitionBy("ProductKey").orderBy(F.col("ingestion_timestamp").desc())
    ...
    spark.sql(f"""
        ...
        ON target.ProductKey = updates.ProductKey
        WHEN MATCHED THEN
          UPDATE SET *
...
df_transformed_stream = transform_products_silver_tier(df_bronze_stream)

query = df_transformed_stream.writeStream \
    .foreachBatch(process_products_micro_batch) \
