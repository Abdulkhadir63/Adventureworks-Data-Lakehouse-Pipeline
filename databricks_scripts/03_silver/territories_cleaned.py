...
def transform_territories_silver_tier(df_raw):
    """
    Applies explicit type casting, standards mapping, string trimming, 
    and appends silver metadata for Territories records.
    """
    logger.info("Applying territories table transformation rules")
    return df_raw \
        .withColumn("SalesTerritoryKey", F.col("SalesTerritoryKey").cast(IntegerType())) \
        .withColumn("Region", F.coalesce(F.trim(F.initcap(F.col("Region"))), F.lit("Unknown"))) \
        .withColumn("Country", F.coalesce(F.trim(F.initcap(F.col("Country"))), F.lit("Unknown"))) \
        .withColumn("Continent", F.coalesce(F.trim(F.initcap(F.col("Continent"))), F.lit("Unknown"))) \
...
if not spark.catalog.tableExists(target_table):
    ...
    empty_transformed_df = transform_territories_silver_tier(empty_schema_df)
...
# -------------------------------------------------------------------------
def process_territories_micro_batch(df_micro_batch, batch_id):
    ...
    df_validated_batch = df_micro_batch.filter(
        F.col("SalesTerritoryKey").isNotNull()
    )

    # WINDOW DEDUPLICATION ON PRIMARY KEY (SalesTerritoryKey)
    window_spec = Window.partitionBy("SalesTerritoryKey").orderBy(F.col("ingestion_timestamp").desc())
    ...
    spark.sql(f"""
        ...
        ON target.SalesTerritoryKey = updates.SalesTerritoryKey
        WHEN MATCHED THEN
          UPDATE SET *
...
df_transformed_stream = transform_territories_silver_tier(df_bronze_stream)

query = df_transformed_stream.writeStream \
    .foreachBatch(process_territories_micro_batch) \
