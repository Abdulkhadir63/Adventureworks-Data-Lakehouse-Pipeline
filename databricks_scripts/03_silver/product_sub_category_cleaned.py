...
def transform_product_subcategory_silver_tier(df_raw):
    """
    Applies explicit type casting, standards mapping, string trimming, 
    and appends silver metadata for Product Subcategory records.
    """
    return df_raw \
        .withColumn("ProductSubcategoryKey", F.col("ProductSubcategoryKey").cast(IntegerType())) \
        .withColumn("ProductCategoryKey", F.col("ProductCategoryKey").cast(IntegerType())) \
        .withColumn("SubcategoryName", F.trim(F.initcap(F.coalesce(F.col("SubcategoryName"), F.lit("Unknown"))))) \
...
if not spark.catalog.tableExists(target_table):
    ...
    empty_transformed_df = transform_product_subcategory_silver_tier(empty_schema_df)
...
# -------------------------------------------------------------------------
def process_product_subcategory_micro_batch(df_micro_batch, batch_id):
    ...
    df_validated_batch = df_micro_batch.filter(
        F.col("ProductSubcategoryKey").isNotNull()
    )

    # WINDOW DEDUPLICATION ON PRIMARY KEY (ProductSubcategoryKey)
    window_spec = Window.partitionBy("ProductSubcategoryKey").orderBy(F.col("ingestion_timestamp").desc())
    ...
    spark.sql(f"""
        ...
        ON target.ProductSubcategoryKey = updates.ProductSubcategoryKey
        WHEN MATCHED THEN
          UPDATE SET *
...
df_transformed_stream = transform_product_subcategory_silver_tier(df_bronze_stream)

query = df_transformed_stream.writeStream \
    .foreachBatch(process_product_subcategory_micro_batch) \
