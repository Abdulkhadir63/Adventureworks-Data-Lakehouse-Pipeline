...
def transform_product_category_silver_tier(df_raw):
    ...
    and appends a silver processing timestamp for Product Category records.
    """
    return df_raw \
...
if not spark.catalog.tableExists(target_table):
    ...
    empty_transformed_df = transform_product_category_silver_tier(empty_schema_df)
...
# -------------------------------------------------------------------------
def process_product_category_micro_batch(df_micro_batch, batch_id):
    ...
    df_validated_batch = df_micro_batch.filter(
        F.col("ProductCategoryKey").isNotNull()
    )

    # WINDOW DEDUPLICATION ON PRIMARY KEY (ProductCategoryKey)
    ...
    spark.sql(f"""
        ...
        ON target.ProductCategoryKey = updates.ProductCategoryKey
        WHEN MATCHED THEN
          UPDATE SET *
...
df_transformed_stream = transform_product_category_silver_tier(df_bronze_stream)

query = df_transformed_stream.writeStream \
    .foreachBatch(process_product_category_micro_batch) \
